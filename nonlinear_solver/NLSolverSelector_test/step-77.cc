...
namespace Step77
{
  ...
  using NLSolve = NLSolverSelector<Vector<double>>;
  ...
  void MinimalSurfaceProblem<dim>::run()
  {
    ...
    for (unsigned int refinement_cycle = 0; refinement_cycle < 4;
         ++refinement_cycle)
      {
        ...
        {
          // NLSolverSelector Testing:
          int test_num = 4; // 0 - KINSOL test
                            // 1 - NOX test
                            // 2 - select() test
                            // 3 - get_data() KINSOL
                            // 4 - get_data() NOX

          // KINSOL/NOX additional data:
          typename NLSolve::AdditionalData additional_data;
          typename kinsol::AdditionalData additional_data_kinsol;
          typename nox::AdditionalData additional_data_nox;

          // NOX parameters
          // NOX stuff
          const Teuchos::RCP<Teuchos::ParameterList> parameters 
            = Teuchos::rcp(new Teuchos::ParameterList);
          const Teuchos::RCP<Teuchos::ParameterList> parameters_nox 
            = Teuchos::rcp(new Teuchos::ParameterList);

          if (test_num == 0)
          {
            // Standard test to see if it works for KINSOL
            // Define solver method          
            std::string solver_type = "kinsol";
            additional_data.solver_type = solver_type;

            // Solver strategy
            additional_data.strategy = NLSolve::AdditionalData::linesearch;
          
            // Additional parameters
            additional_data.maximum_non_linear_iterations = 100;
          }
          else if (test_num == 1)
          {
            // Test to see if it works nicely for NOX
            std::string solver_type = "nox";
            additional_data.solver_type = solver_type;

            // Additional parameters
            additional_data.maximum_non_linear_iterations = 20;

            // Some parameters to get more specific
            parameters->set("Nonlinear Solver","Line Search Based");
            Teuchos::ParameterList& Line_Search = 
              parameters->sublist("Line Search");
            Line_Search.set("Method","Full Step");
          }
          else if (test_num == 2)
          {
            /**
             * Here we test the select() function
             * We just set the nonlinear solver to nox at first,
             * then we'll switch to kinsol before we run anything.
            */
            ...
            // Additional parameters
            additional_data.maximum_non_linear_iterations = 50;
          }
          else if (test_num == 3)
          {
            /**
             * Here we will test the get_data function for
             * the kinsol nonlinear solver.
             * In other words, define a kinsol data type and throw
             * it into the NLSolverSelector class.
            */

            std::string solver_type = "kinsol";
            additional_data.solver_type = solver_type;

            additional_data_kinsol.maximum_non_linear_iterations = 60;
          }
          else if (test_num == 4)
          {
            /**
             * Here we do the same thing as test 3 except
             * we use nox instead. We will import data configured
             * for NOX for both the additionaldata class and the
             * parameters class (although that one doesn't change really)
            */
            ...
            // Additional parameters
            additional_data_nox.max_iter = 15;

            // Some parameters to get more specific
            parameters_nox->set("Nonlinear Solver","Line Search Based");
            Teuchos::ParameterList& Line_Search = 
              parameters_nox->sublist("Line Search");
            // Below is the difference to look for in the output.
            Line_Search.set("Method","Backtrack");
          }

          NLSolverSelector<Vector<double>> nonlinear_solver(additional_data,parameters);

          if (test_num == 2)
          {
            std::string new_solver_type = "kinsol";
            nonlinear_solver.select(new_solver_type);
          }
          else if (test_num == 3)
          {
            nonlinear_solver.set_data(additional_data_kinsol);
          }
          else if (test_num == 4)
          {
            nonlinear_solver.set_data(additional_data_nox,parameters_nox);
          }
